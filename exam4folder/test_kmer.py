...
def test_update_count_is_correct():
    """Count should be 1 after seeing a k-mer once, fails when bug is present in
    og code, should pass if files are correct in revision."""
    data = update_kmer_count({}, "ACGT", "A")
...
def test_main_multiple_sequences(tmp_path):
    """All sequences should appear in output, fails when bug is present in og code
    before revision."""
    import sys
